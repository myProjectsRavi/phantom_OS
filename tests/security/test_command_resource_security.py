...
def _allowed(command) -> bool:
    ...
    return policy.allow(ActionRequest(type=PhantomActionType.RUN_COMMAND, params={"command": command}))


@pytest.mark.parametrize("command", [["ls", "~/.ssh"], ["ls", "/etc"], ["ls", "-R", "/"], ["open", "file:///etc/passwd"], ["open", "ftp://example.com/file"], ["screencapture", "/tmp/capture.png"], ["tesseract", "/tmp/input.png", "/tmp/output"], ["/bin/ls", "/tmp"]])
def test_resource_aliases_and_unneeded_commands_fail_closed(command):
    assert _allowed(command) is False


@pytest.mark.parametrize("command", [["ls"], ["ls", "-la", "/tmp"], ["echo", "hello"], ["date"], ["pwd"], ["open", "https://example.com"], ["open", "-a", "Safari"], ["killall", "Slack"]])
