...
def test_path_traversal_into_etc_is_blocked():
    assert (
        SafetyPolicy().allow(
            ActionRequest(type=PhantomActionType.FILE_OPEN, params={"path": "../../etc/passwd"})
        )
        is False
    )
...
def test_sensitive_home_path_is_blocked():
    assert (
        SafetyPolicy().allow(
            ActionRequest(type=PhantomActionType.FILE_OPEN, params={"path": "~/.ssh/config"})
        )
        is False
    )
...
def test_configured_sensitive_domains_are_blocked(url):
    ...
    assert (
        policy.allow(ActionRequest(type=PhantomActionType.URL_OPEN, params={"url": url})) is False
    )
...
def test_three_native_failures_trip_circuit_breaker(monkeypatch):
    ...
    monkeypatch.setattr(
        executor,
        "_dispatch",
        lambda request: ActionResult(
            success=False, action_type=request.type, error="native failure"
        ),
    )
    ...
    blocked = asyncio.run(executor.execute(request))
