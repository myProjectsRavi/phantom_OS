...
def test_init_creates_private_local_layout(tmp_path, monkeypatch):
    ...
    assert (
        (root / "recipes").exists() and (root / "logs").exists() and (root / "config.toml").exists()
    )
...
def test_stop_without_live_control_cleans_stale_state(tmp_path, monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(cli.Path, "home", lambda: tmp_path)
    control = tmp_path / ".phantom" / "phantom.sock"
    monkeypatch.setattr(cli, "socket_path", lambda: control)

    def unavailable(_command):
        raise cli.DaemonUnavailable("not running")

    monkeypatch.setattr(cli, "send_command", unavailable)
    root = tmp_path / ".phantom"
    root.mkdir(parents=True)
    pid_file = root / "phantom.pid"
...
def test_stop_signals_pid_proven_by_live_control(tmp_path, monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(cli.Path, "home", lambda: tmp_path)
    monkeypatch.setattr(cli, "send_command", lambda command: {"ok": True, "pid": 1234})
    sent = []
    monkeypatch.setattr(cli.os, "kill", lambda pid, sig: sent.append((pid, sig)))
    result = runner.invoke(cli.main, ["stop"])
    assert result.exit_code == 0
    assert sent == [(1234, 0), (1234, signal.SIGTERM)]
    pid_file = tmp_path / ".phantom" / "phantom.pid"
...
def test_stop_repairs_stale_pid_and_never_signals_it(tmp_path, monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(cli.Path, "home", lambda: tmp_path)
    monkeypatch.setattr(cli, "send_command", lambda command: {"ok": True, "pid": 5678})
    pid_file = tmp_path / ".phantom" / "phantom.pid"
    ...
    assert result.exit_code == 0
    assert (
        sent == [(5678, 0), (5678, signal.SIGTERM)]
        and all(pid != 1234 for pid, _sig in sent)
        and pid_file.read_text() == "5678"
    )
...
def test_stop_rejects_invalid_live_process_identity(tmp_path, monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(cli.Path, "home", lambda: tmp_path)
    monkeypatch.setattr(cli, "send_command", lambda command: {"ok": True, "pid": "not-a-pid"})
    sent = []
    monkeypatch.setattr(cli.os, "kill", lambda pid, sig: sent.append((pid, sig)))
    result = runner.invoke(cli.main, ["stop"])
    assert result.exit_code != 0
    assert "invalid process identity" in result.output
    assert sent == []
...
def test_status_and_stats_commands(monkeypatch):
    ...
    def fake_daemon(command, **_payload):
        if command == "status":
            return {"ok": True, "status": {"running": True, "stats": {"frames_processed": 1}}}
        if command == "stats":
            return {
                "ok": True,
                "stats": {
    ...
    assert (
        runner.invoke(cli.main, ["status"]).exit_code == 0
        and runner.invoke(cli.main, ["stats"]).exit_code == 0
    )
...
def test_trust_emergency_resume_and_undo_use_daemon(monkeypatch):
    ...
    def fake_daemon(command, **payload):
        ...
        return {"ok": True, "undone": True} if command == "undo" else {"ok": True}

    monkeypatch.setattr(cli, "_daemon", fake_daemon)
    for args in (["trust", "approve_each"], ["emergency-stop"], ["resume-actions"], ["undo"]):
        assert runner.invoke(cli.main, args).exit_code == 0
    assert calls == [
        ("trust", {"level": "approve_each"}),
        ("emergency_stop", {}),
        ("resume", {}),
        ("undo", {}),
    ]
...
def test_recipe_commands_use_interactive_agent(monkeypatch):
    ...
    class _Agent:
        def run_recipe(self, name):
            state["run_name"] = name
            return {"success": True, "duration_ms": 8}

        def list_recipes(self):
            return [
                SimpleNamespace(
                    name="demo", source="builtin", trigger=None, run_count=2, enabled=True
                )
            ]
