...
def test_perceive_intent_switch_and_patterns(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        agent._intent,
        "recognize",
        lambda frame: IntentResult(
            intent=IntentType.CODING, confidence=0.9, source_app=frame.app_name
        ),
    )
...
def test_execute_and_action_wrappers(tmp_path, monkeypatch):
    ...
    base = asyncio.run(
        agent.execute(
            ActionRequest(
                type=PhantomActionType.TYPE_TEXT, params={"text": "manual"}, source="test"
            )
        )
    )
    ...
    assert [r.type for r in captured[-3:]] == [
        PhantomActionType.TYPE_TEXT,
        PhantomActionType.APP_ACTIVATE,
        PhantomActionType.URL_OPEN,
    ]
...
def test_sync_wrappers_inside_running_loop(tmp_path, monkeypatch):
    ...
    async def _run():
        return agent.type_text("inside-loop")
...
def test_recipe_clipboard_and_safety_helpers(tmp_path, monkeypatch):
    ...
    class _Clipboard:
        ...
        def history(self, limit=20):
            return [{"content": self._value}] * min(limit, 1)

    agent._executor._clipboard = _Clipboard()
    agent._executor._history = [1, 2, 3]
    monkeypatch.setattr(
        agent._executor,
        "undo_last",
        lambda: ActionResult(success=True, action_type=PhantomActionType.WAIT),
    )
    ...
    monkeypatch.setattr(
        agent._triggers, "check", lambda _: [Recipe(name="match", source="builtin")]
    )
    ...
    async def _fake_run(_recipe, _variables):
        return {"success": True, "duration_ms": 4}

    monkeypatch.setattr(agent._runner, "run", _fake_run)
    stored = []
    agent._neurovault = SimpleNamespace(
        available=True, store_workflow=lambda name, steps: stored.append((name, steps))
    )
    assert asyncio.run(agent.run_recipe("demo"))["success"] is True
    assert stored == [("demo", ["wait"])]
    adds = []
    saves = []
    monkeypatch.setattr(agent._recipes, "add", lambda rec: adds.append(rec.name))
    monkeypatch.setattr(agent._recipes, "save", lambda rec: saves.append(rec.name))
    ...
    agent.enable_recipe("toggle")
    assert toggle.enabled is True
    agent.disable_recipe("toggle")
    assert toggle.enabled is False
    agent.set_trust_level(TrustLevel.APPROVE_EACH)
    assert agent.trust_level == TrustLevel.APPROVE_EACH
    agent.emergency_stop()
    assert agent._safety.is_stopped is True
