...
def test_predict_orders_by_transition_count_and_caps_top_three():
    predictor = AppContextPredictor()
    for _ in range(5): predictor.observe("Code", "Slack", 10)
    for _ in range(3): predictor.observe("Code", "Terminal", 20)
    for _ in range(2): predictor.observe("Code", "Browser", 5)
...
def test_predict_remaining_time_floors_at_zero_and_uses_average():
    predictor = AppContextPredictor(); predictor.observe("Mail", "Calendar", 10); predictor.observe("Mail", "Calendar", 30)
...
def test_observe_caps_duration_history():
    predictor = AppContextPredictor()
    for i in range(120): predictor.observe("Code", "Slack", float(i))
