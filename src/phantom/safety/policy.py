...
class SafetyPolicy:
    ...
    BLOCKED_APPS = frozenset(
        ["1Password", "Keychain Access", "System Preferences", "System Settings", "Disk Utility"]
    )
        ...
        *,
        ...
        if approval_store_path is None:
            ...
            approval_store_path = (
                ...
                else Path.home() / ".phantom" / "approvals.json"
            )
        self._approval_store_path = (
            Path(approval_store_path).expanduser() if approval_store_path else None
        )
    ...
    def _approval_key(self, request: ActionRequest) -> str:
        try:
            params = json.dumps(request.params, sort_keys=True, separators=(",", ":"), default=str)
        except (TypeError, ValueError):
            params = repr(request.params)
    ...
    def _load_approval_store(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            if not force and stat_result.st_mtime_ns == self._approval_store_mtime_ns:
                return
            os.chmod(path, 0o600)
            ...
            self._approval_store_mtime_ns = None
    ...
    def _persist_approval_store(self) -> None:
        ...
        try:
            ...
            if temp_path is None:
                raise RuntimeError("Failed to create approval store temporary file")
    ...
    def _check_rate(self):
        now = time.time()
        self._rate = [timestamp for timestamp in self._rate if now - timestamp < 60]
        if len(self._rate) >= self._max_actions_per_minute:
            return False
    ...
    def _check_blocklists(self, request: ActionRequest) -> bool:
        if request.type == PhantomActionType.SEQUENCE:
            ...
            for step in steps:
                ...
                try:
                    ...
                    return False
                if not self._check_blocklists(child):
                    return False
            return True
        app = str(request.params.get("app", "") or "")
        if self._is_blocked_app(app):
            return False
        path = str(request.params.get("path", "") or "")
        if path and self._is_blocked_path(path):
            return False
        command_value = request.params.get("command", "")
        text_value = request.params.get("text", "")
        ...
        if request.type == PhantomActionType.RUN_COMMAND:
            ...
            executable = argv[0]
            targets = argv[1:]
            if executable == "killall" and not self._killall_targets_allowed(targets):
                return False
            if executable == "open" and not self._open_targets_allowed(targets):
                return False
            if executable == "ls" and not self._ls_targets_allowed(targets):
                return False
        url = str(request.params.get("url", "") or "")
        return not self._is_blocked_url(url)

    def _killall_targets_allowed(self, args: list[str]) -> bool:
        if not args or any(not arg or arg.startswith("-") for arg in args):
            return False
    ...
    def _open_targets_allowed(self, args: list[str]) -> bool:
        ...
        while index < len(args):
            ...
            if arg == "-a":
                if index + 1 >= len(args):
                    return False
                if self._is_blocked_app(args[index + 1]):
                    return False
                index += 2
                continue
            if arg.startswith("-"):
    ...
    def _normalize_app_name(app: str) -> str:
        return unicodedata.normalize("NFKC", str(app)).strip().casefold()
    ...
    def _is_blocked_path(self, path: str) -> bool:
        ...
        for blocked in self.blocked_paths:
            try:
                root = Path(blocked).expanduser().resolve(strict=False)
            except (OSError, RuntimeError, ValueError):
                continue
    ...
    def _command_text(raw: object) -> str:
        return " ".join(str(part) for part in raw) if isinstance(raw, list) else str(raw or "")
    ...
    def _is_blocked_url(self, url: str) -> bool:
        if not url or not self.blocked_domains:
            return False
